...
# 按照题材（文学或者儿童）筛选，绘制女性词汇趋势. 可以在17.6中看到提及
...
```{r}
# analyze keyworsds
edbf_term_counts_genre$womword<-as.integer(grepl("women|feminist|feminism|gender|harassment|sexism|sexist", x = edbf_term_counts_genre$word))

head(edbf_term_counts_genre)
```


```{r}
#compute aggregate statistics; choose children
...
race_keywords <- "race|racis.*|diversity|multicultural|ethnic.*|minority|diaspora|black|white"
# 标记关键词并计算整体频率
edbf_counts_race <- edbf_term_counts_race %>%
  mutate(raceword = as.integer(grepl(race_keywords, word))) %>%
  group_by(year) %>%
  mutate(year_total = sum(n)) %>%  # 计算词汇总量
  filter(raceword == 1) %>%  #保留匹配关键词
...
ggplot(edbf_counts_race, aes(year, sum_race / year_total, group=1)) + 
  geom_line() +
  xlab("Year") +
